...
describe('POST /PostTimeTable (e2e)', () => {
	const pool = createPool('localhost', process.env.POSTGRES_USER as string, process.env.POSTGRES_PASSWORD as string)
	...
	it('should upsert timetable without duplicates on repeated calls', async () => {
		...
		const startIso = new Date(Date.UTC(2030, 0, 1, 10, 0, 0)).toISOString().replace(/\.\d{3}Z$/, '+00:00')

		const payload = {
			Doctors: [{ Id: doctorId, Name: `Доктор ${doctorId}` }],
			Branches: [{ Id: branchId, Name: `Филиал ${branchId}` }],
			Intervals: [
				{
					DoctorId: doctorId,
					BranchId: branchId,
